...
int main()
{
    int i,j,k,l,m,n,o,p,arra[100][100],arrb[100][100],sum[100][100];
    ...
    for(i=1;i<=m;i++)
    {
        for(j=1;j<=n;j++)
        {
            printf("Enter the %d,%dth Number in First Matrix: ",i,j);
            scanf("%d",&arra[i][j]);
        }
    }
    for(k=1;k<=m;k++)
    {
        for(l=1;l<=n;l++)
        {
            printf("Enter the %d,%dth Number in Second Matrix: ",k,l);
            scanf("%d",&arrb[k][l]);
        }
    }
    for(i=1;i<=m;i++)
